...
if __name__ == "__main__":
    ...
    print(f"Eval Accuracy (sign): {accuracy:.4f}")
    print(f"Eval Bias (mean error): {bias:.6f}")
    print(f"Eval Variance (error var): {variance:.6f}")

    if args.export_head:
        torch.save(model.head.state_dict(), args.export_head)
        print(f"Saved head weights to {args.export_head}")